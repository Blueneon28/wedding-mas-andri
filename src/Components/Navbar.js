...
const Navbar = ({ toggle }) => {
  ...
  const GetID = () => {
    ...
    if (textCode === "") {
      ...
      axios
        .get(
          "https://api.wedding.chicodefive.com/v1/User/generateidforqr/" +
            textCode
        )
        .then((res) => {
  ...
  const checkHadiran = (textCek) => {
    ...
    axios
      .get("https://api.wedding.chicodefive.com/v1/User/checkhadir/" + textCek)
      .then((resp) => {
  ...
  const CekKehadiran = useCallback(() => {
    ...
    if (textCek === "0") {
      ...
      axios
        .get(
          "https://api.wedding.chicodefive.com/v1/User/checkhadir/" + textCek
        )
        .then((resp) => {
          if (typeof resp.data == "object") {
            // setOpenQR(!openQR)
                            ...
                            <div className="flex flex-col w-auto h-auto justify-center items-center">
                              <p className="font-sans font-light text-red-500 text-xs text-left md:px-24 px-4">
                                * Nomor handphone kamu salah atau tidak
                                ...
                                tidak bisa silahkan menghubungi Admin
                              </p>
                            </div>
                          )}
                        </div>
